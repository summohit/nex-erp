...
import {
  ...
  LucideRotateCcw, LucideWandSparkles, LucideTrash2,
} from '@lucide/angular';
import { ShiftsService, RosterCell, RosterGrid, RosterRow, RosterShift } from '../../services/shifts.service';
...
export class ShiftRosterComponent implements OnInit {
  ...
  cellTitle(row: RosterRow, cell: RosterCell): string {
    ...
    if (cell.type === 'SHIFT') return `${who}: ${cell.shift?.name} ${this.cellTime(cell)}${cell.isDefault ? ' (default shift)' : ''}`;
  ...
  submitBulk() {
    ...
    this.shiftsService.bulkAssignRoster({
      ...
      shiftId: this.bulkForm.isDayOff ? null : Number(this.bulkForm.shiftId),
